import time
import os
ROOT = os.path.dirname(__file__)#os.path.dirname(__file__))
...
import assembly
from device import SerialCommsManager as Robot
from scipy.spatial.transform import Rotation

import js2pyrtc.robot as pyrtc
import js2pyrtc.my_track as pyrtc_tracks

######################################
##          OVERLORD
######################################

OVERLORD_IP = "192.241.156.85"

def get_ip():
    # try:
      # endpoint = 'https://ipinfo.io/json' # global ip
...
def heartbeat(name, ipv4, dtype="robot"):
  return requests.post(f"http://{OVERLORD_IP}/heartbeat",
    json={"name": name, "ipv4": ipv4, "dtype": dtype}).text

def heartbeat_handler(name):
  ipv4 = get_ip()
  while True:
    heartbeat(name, ipv4)
    time.sleep(0.25)
...
def imshow(buf, dtype="color"):
  global _camera_buffer
  _camera_buffer.set(buf, dtype=dtype)
...
def _rpc_robot_values():
  ...
  sensors = []
  _robot_lock.acquire()
  for mvals in _motor_value_list:
    motors += mvals[:]
  for svals in _sensor_value_list:
    sensors += svals[:]
  _robot_lock.release()
  return {
    "motors": motors,
...
def _rpc_obj3d():
  global _mates_obj, _mates_lock
  _mates_lock.acquire()
  model = [assembly.getJson(mate) for mate in _mates_obj]
  _mates_lock.release()
  return model
...
def _run_process(logbuf, lock, lasttime, en, cambuf, gpos, kboard, kvalues,
    gpbtns, gpaxes, bboxes, bblock, mlist, mlock, rlock, ndev, *args):
  ...
  global _global_robot, _robot_lock
  _robot_lock = rlock

  global _motor_value_list, _sensor_value_list
  for i in range(ndev):
    _motor_value_list.append(args[i])
    _sensor_value_list.append(args[i + ndev])
...
def init(robot=None, frame_size=(640, 360), name="Anonymous Robot"):
  global _initialized
  if _initialized:
    return
  _initialized = True
  ...
  global _camera_buffer
  _camera_buffer = pyrtc_tracks.DoubleFramebuffer(*frame_size)
  ...
  if robot is not None:
    _motor_value_list, _sensor_value_list = robot._value_lists()
    num_devices = len(_motor_value_list)
  ...
  main_process = Process(target=_run_process, args=(
    ...
    _mates_obj, _mates_lock,
    _robot_lock, num_devices,
    *_motor_value_list, *_sensor_value_list
  ))
  main_process.start()
  # heartbeat(name, get_ip() + ":8080", dtype="robot")
